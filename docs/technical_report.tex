...
Figure~\ref{fig:snap-ours-rand} shows the four canonical lattice states
visited by this schedule on a near-critical random input: initial
configuration, after the deterministic preprocessing phase
\(F_0^{L/2}\), after the two shake cycles, and after the final
amplification phase~\(M_{\mathrm{Moore81}}^{4L}\).  The
preprocessor leaves a finely-mottled state that is roughly density-uniform
on a coarse scale, the shake cycles re-randomise it without altering
density, and the final Moore-81 sweep collapses it to consensus.
