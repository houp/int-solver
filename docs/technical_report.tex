...
The open follow-ups remain:
\emph{(i)} extend the adversarial battery beyond the eight families
considered here, or use a stronger structural search (e.g., active
adversarial generation against the augmented classifier);
\emph{(ii)} multi-seed autotune at multiple \(L\), starting near the
top single-seed find of Section~\ref{subsec:schedule-autotune} (the
\fpath{classifier/schedule_autotune.py} harness already supports
\fpath{--alpha-near-baseline} for this purpose);
\emph{(iii)} formalise the convergence of the rebalanced mixture
(Section~\ref{subsec:strict-ca-theory} sketches what's needed).
