...
mode.  We therefore retain the rebalanced mixture as the
recommendation and add the parameterized battery to the project's
permanent regression suite.

\paragraph{Open follow-ups.}
With tier-3 settled, the remaining open follow-ups are:
\emph{(i)} multi-seed autotune at multiple \(L\), starting near the
...
\emph{(ii)} a structurally-aware extension of
...
breakdown above);
\emph{(iii)} prove (or refute) Conjecture~\ref{conj:two-phase} on
\(\Theta(\log \delta^{-1}) + \Theta(L)\) absorption.
...
\begin{conjecture}[two-phase \(\log + L\) absorption]
\label{conj:two-phase}
...
\[
\mathbb{E}[\tau] \;=\; \Theta\bigl(\log \delta^{-1}\bigr) + \Theta(L),
\qquad
...
The first term is the mean-field bias-amplification phase formalised
in Proposition~\ref{prop:meanfield-rate}; the second term is the
coarsening phase observed empirically in
Table~\ref{tab:strict-ca-validation}; the exponential
correct-consensus probability matches the form of Hoeffding's
inequality on the initial-density Bernoulli sum and is consistent
with the empirically-observed paired-McNemar separation between
mixtures that we reported in Section~\ref{subsec:h2h-benchmark}.  We have not
proved any of the three claims rigorously, but each is a precise
prediction that subsequent theoretical work could attempt to
confirm or refute.
