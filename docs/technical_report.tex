...
\(L = 128,\,\delta = 0.02\):
\begin{itemize}
\item \texttt{baseline\_published}: the published 6-rule strict-CA mixture
  with weights \((0.70,\,4{\times}0.05,\,0.10)\)
...
\item \texttt{combined\_r\_break\_plus\_rebalanced}: the 7-rule
  ...
  weights, weights
  \((0.04,\,0.05,\,0.12,\,0.02,\,0.55,\,0.17,\,0.05)\).
\end{itemize}
...
\texttt{classifier/head\_to\_head\_benchmark.py}.  Runtime: \(34\) min
on M2 Max.

\paragraph{Results: per-candidate cc with 95\% Wilson CIs.}
Result file:
\texttt{results/density\_classification/2026-04-30/h2h\_tier1.json}.
...
\paragraph{Limitations and follow-up.}
Two things this benchmark did not test:
\emph{(i)} adversarial inputs at \(\delta = 0.005\) or below, where
the strict-CA gap to the non-local baseline is largest
(Section~\ref{subsec:rho-min}); the extended adversarial battery
should be re-run at the tighter density;
\emph{(ii)} the same head-to-head at \(L \in \{64, 96, 192, 256\}\),
to verify that the rebalanced-weights advantage is universal across
grid sizes rather than a feature of \(L=128\).  Both are natural
follow-ups; the harness
(\texttt{classifier/head\_to\_head\_benchmark.py}) is parametric and
ready to run them.
