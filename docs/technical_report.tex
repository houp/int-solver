...
\paragraph{Strict-CA recommendation, finalized.}
The strict-CA density classifier is now recommended in two flavours
chosen by workload:
\begin{itemize}
\item \emph{Random-heavy default} (e.g., uniformly sampled
  Bernoulli inputs at \(L \geq 96\)): the \(6\)-rule
  \texttt{rebalanced\_v2\_weights} mixture
  \[
  \bigl(F_0,\,\mathrm{traffic}_{NE},\,\mathrm{traffic}_{NW},\,
         \mathrm{traffic}_{SE},\,\mathrm{traffic}_{SW},\,M_{\mathrm{Moore9}}\bigr),
  \]
  with weights \((0.041,\, 0.038,\, 0.120,\, 0.016,\, 0.624,\,
  0.161)\).
\item \emph{Adversarial-heavy variant} (e.g., \(L < 96\) or
  structured priors): the \(10\)-rule \texttt{set\_B} mixture, which
  augments the above with the four cardinal traffic rules
  \(\mathrm{traffic}_{N/S/E/W}\); the cardinal-traffic block carries
  half of \(p_M\) (each rule gets \(0.0235\)) and the remaining half
  of \(p_M\) is \(M_{\mathrm{Moore9}}\)'s share.  The price is a
  \(\sim 3\) pp regression on random correctness at \(L = 128\); the
  benefit is \(+5.8\) pp on the parameterized battery's symmetric
  min cc.
\end{itemize}
...
With tier-4 settled, the remaining open follow-ups are:
\emph{(i)} a 10-rule autotune over set \(B\)'s simplex --- the cardinal
weights here are the naive equal-share template, not optimized;
\emph{(ii)} a structurally-aware extension of
Proposition~\ref{prop:meanfield-rate} that handles spatially
correlated priors (the failure mode identified by the
per-family breakdown);
\emph{(iii)} prove (or refute) Conjecture~\ref{conj:absorb-time} on
\(\mathbb{E}[\tau] = \Theta(L^{\alpha} \delta^{-\beta})\) absorption.
