...
Figure~\ref{fig:snap-ours-rand} shows all eight canonical lattice
states visited by the schedule on a near-critical random input:
initial configuration, after the deterministic preprocessing phase
\(F_0^{L/2}\), after each of the three sub-phases of each shake cycle
(amplifier, then \(F_0^{L/4}\) re-mix, then random pair-swap), and
after the final amplification phase~\(M_{\mathrm{Moore81}}^{4L}\).
On easy near-critical random inputs the lattice is already at
consensus by the end of the first shake's amplifier; the rest of the
schedule simply preserves that consensus.

\begin{figure}[h]\centering
\includegraphics[width=\linewidth]{figures/snapshots_ours_random.pdf}
\caption{Snapshots of the L-scaled schedule on a random
\(L=128\), \(\rho=0.51\) input.  Density is conserved
through all but the final amplification phase (where the lattice
collapses to all-1).  Black = 1, white = 0.}
\label{fig:snap-ours-rand}
\end{figure}

\begin{figure}[h]\centering
\includegraphics[width=\linewidth]{figures/snapshots_ours_stripes.pdf}
\caption{Snapshots of the L-scaled schedule on a structured
\(L=128\), \(\rho=0.55\) horizontal-stripes input.  The
preprocessor immediately destroys the stripe alignment; subsequent
shake cycles ensure no axial structure regrows; the amplifier
finalises to majority-color consensus.}
