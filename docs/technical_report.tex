...
\paragraph{Status.}
A 200-rule smoke test ran end-to-end in \({\sim}3.5\) minutes
and produced top candidates with adversarial cc \(>0.90\) at
\(L=64\), \(\delta=0.05\) --- already on par with the published
mixture from Section~\ref{subsec:strict-ca}.  This validates the
methodology on a small scale.  The production run at
\(K = 50{,}000\), \(N_1 = 100\), \(N_2 = 8\) was launched
(\texttt{results/density\_classification/2026-04-29/r\_break\_search\_50k.json})
but interrupted before stages~2/3 finished.  Resuming the search ---
either at the same scale with stdout flushing for visible
incremental progress, or at a smaller scale to fit within a typical
\(30\)~min budget --- is the natural next step.  We retain the cheap
top-200 probe results
(\texttt{results/density\_classification/2026-04-29/r\_break\_probe\_5k.json}
and \texttt{r\_break\_probe\_10k.json}) as a starting point for the
follow-up search.
