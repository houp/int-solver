...
\paragraph{Open follow-ups, after tier-5.}
Tier-5 closed the 10-rule autotune follow-up.  The remaining open
items are:
...
specifically the topological-defect-lifetime contribution to the
\(\delta^{-1}\) factor;
\emph{(iii)} explore an even larger rule pool (e.g., adding the
remaining 4 cardinal shifts to the 10-rule autotune pool, giving 14
rules) under a longer autotune budget; the structure-breaker probe
suggested set \(D\) (14 rules) was non-dominant at the
equal-share template, but it is plausible that an autotuned 14-rule
mixture lies above the current Pareto frontier.
